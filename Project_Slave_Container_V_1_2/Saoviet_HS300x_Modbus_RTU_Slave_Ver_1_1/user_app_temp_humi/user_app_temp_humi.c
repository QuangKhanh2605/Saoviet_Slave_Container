		...
		if(CRC_check == CRC_rx)
		{
			...
			else if(FunCode == 0x06)
			{
				if(addr_data == 0x0000)
				{
					...
					{
						*addr_stm32l0xx = addr;
						FLASH_WritePage(FLASH_ID_BAURATE, *addr_stm32l0xx, *baud_rate);
...
void Change_Baudrate_AddrSlave_Calib_ATCommand(UART_BUFFER *sUart2, uint8_t *addr_stm32l0xx, uint32_t *baud_rate, int16_t *drop_tem, int16_t *drop_humi)
{
	...
	receive_ctrl = Terminal_Receive(sUart2, *addr_stm32l0xx);
	if(receive_ctrl == -1)
	{
	...
	else if(receive_ctrl == 1)
	{
		...
		*baud_rate=BAUDRATE_DEFAULT;
		...
		HAL_UART_Receive_IT(sUart2->huart,&sUart2->buffer,1);
		FLASH_WritePage(FLASH_ID_BAURATE, *addr_stm32l0xx, *baud_rate);
        FLASH_WritePage(FLASH_CALIB_SENSOR, *drop_tem, *drop_humi);
	}
	...
	else if(receive_ctrl == 2)
	{
		uint8_t i=0;
		uint8_t count=0;
        while( i < sUart2->countBuffer)
        {
            if(sUart2->sim_rx[i++] == ' ')  break;
        }
		...
		i--;
		if(count >0 && count <=3 && i == (sUart2->countBuffer - 1))
		{
			uint16_t tmp=0;
	...
	else if(receive_ctrl == 3)
	{
		...
		i--;
		if(count == 1 && i == (sUart2->countBuffer - 1))
		{
	...
	else if(receive_ctrl == 4 || receive_ctrl == -4)
	{
		uint8_t i=0;
		uint8_t count=0;
        while( i < sUart2->countBuffer)
	...
	else if(receive_ctrl == 5 || receive_ctrl == -5)
	{
		uint8_t i=0;
		uint8_t count=0;
        while( i < sUart2->countBuffer)
...
int8_t Terminal_Receive(UART_BUFFER *sUart2, uint8_t addr_stm32l0xx)
{
	...
	if(sUart2->sim_rx[i] == 'I' && sUart2->sim_rx[i+1] == 'D')
	{
        ...
        if(sUart2->sim_rx[i++] == '=') 
        {
            if(Get_Uint_In_String(sUart2->sim_rx, i, sUart2->countBuffer) == addr_stm32l0xx)
            {
                return 2;
            }
        }  
