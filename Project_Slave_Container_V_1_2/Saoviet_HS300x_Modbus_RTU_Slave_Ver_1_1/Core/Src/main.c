...
uint32_t baud_rate      = BAUDRATE_DEFAULT;
uint8_t  addr_stm32l0xx = ID_DEFAULT;
...
int main(void)
{
  ...
  {
    addr_stm32l0xx = FLASH_ReadData32(FLASH_ID_BAURATE+4);
    baud_rate      = FLASH_ReadData32(FLASH_ID_BAURATE+8); 
  }
  
  if(FLASH_ReadData32(FLASH_CALIB_SENSOR) == DEFAUL_READ_FLASH) 
  {
    drop_tem  = FLASH_ReadData32(FLASH_CALIB_SENSOR+4);
    drop_humi = FLASH_ReadData32(FLASH_CALIB_SENSOR+8); 
  }
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		{ 
			...
			{
				Change_Baudrate_AddrSlave_Calib_ATCommand(&sUart2, &addr_stm32l0xx, &baud_rate, &drop_tem, &drop_humi);
				if(ModbusRTU_Slave(&sUart2, &addr_stm32l0xx, &baud_rate, tem, humi) == 1)
				{
		...
		if(HAL_GetTick() - gettick_reset > TIME_WDG_RESET_MCU)
		{
			gettick_reset = HAL_GetTick();
            __disable_irq();
			NVIC_SystemReset();
